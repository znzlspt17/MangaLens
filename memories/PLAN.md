...
| P7 | 로컬 번역 | 외부 API 키 의존 없이 Hunyuan-MT-7B 로컬 모델로 번역함. 서버가 키를 없는 환경에서도 동작 |
| P8 | GPU 자동 감지 | CUDA 또는 ROCm 환경을 자동 감지하여 적절한 백엔드 선택 (현재: NVIDIA RTX 4090 / CUDA 12.8) |
...
│                      │     + 후리가나 제거 (connected component 필터링)
└─────────┬───────────┘
          │ 업스케일된 크롭 이미지
          ▼
┌─────────────────────┐
...
│   (Translation)      │     tencent/Hunyuan-MT-7B, 7B 파라미터
└─────────┬───────────┘
...
### 3.1 말풍선 검출 — comic-text-detector / Magi v2
...
| **x4 기본 변형** | `anime_6B` — 애니메이션/만화 특화 (RRDBNet 6블록, 17MB) |
| **x4 대체 변형** | `x4plus` — 실사 범용 (RRDBNet 23블록, 64MB) |
| **변형 선택** | `UPSCALER_VARIANT` 환경변수 (`anime_6b` 기본, `x4plus` 선택 가능) |
...
### 3.4 번역 — Hunyuan-MT-7B (로컬 LLM)

| 항목 | 내용 |
|------|------|
| **모델** | [tencent/Hunyuan-MT-7B](https://huggingface.co/tencent/Hunyuan-MT-7B) |
| **아키텍처** | CausalLM, 7B 파라미터 (WMT25 경진대회 30/31 언어쌍 1위) |
| **추론 방식** | 싱글턴 + `threading.Lock` 이중 확인, `asyncio.to_thread()` 비동기 |
| **VRAM** | fp16 ~14GB (RTX 4090 권장), CPU fallback 가능 |
| **API 키** | 불필요 — 완전 로컬 인퍼런스 |
| **배치 처리** | 텍스트별 개별 추론 (배치 API 없음) |
...
### 5.3 ~~사용자 API 키 관리~~ → 제거됨 (D-014)

> Hunyuan-MT-7B 로컬 인퍼런스 채택으로 외부 번역 API 키가 불필요해졌습니다.
> `session_store`, `POST/GET /api/settings`, `X-DeepL-Key` / `X-Google-Key` / `X-Session-Id` 헤더 관련 코드가 모두 제거되었습니다.
...
│   │   ├── magi_detector.py        # 말풍선 검출 대체 (Magi v2, USE_MAGI_DETECTOR=true)
...
| `numpy` | 수치 연산 | >=1.26 |
| `httpx` | 비동기 HTTP 클라이언트 (모델/폰트 다운로드) | >=0.28 |
...
| `accelerate` | HuggingFace 모델 로딩 가속 (Magi v2, Hunyuan-MT-7B) | >=1.0 |
| `transformers` | HuggingFace 모델 추론 (Hunyuan-MT-7B) | >=4.43 |
...
- [x] OCR 모듈 (manga-ocr)
- [x] 번역 모듈 (DeepL API → Phase 10에서 Hunyuan-MT-7B로 교체)
- [x] OCR 대조 기록 시스템
...
### Phase 8: 모델 업그레이드 ✅ 완료 (2026-03-26)
- [x] Tier 1: RealESRGAN anime_6B 업스케일러 교체 (기본값)
- [x] Tier 2: Magi v2 통합 감지기 (A/B 토글, 기본 비활성)

### Phase 10: Hunyuan-MT-7B 번역 엔진 마이그레이션 ✅ 완료 (2026-03-26)
- [x] translator.py 완전 재작성 (DeepL/Google → Hunyuan-MT-7B 로컬 인퍼런스)
- [x] DeepL/Google API 키 관련 코드 전면 제거 (config, schemas, routers)
- [x] server/routers/settings.py 삭제
- [x] orchestrator.py Translator() 호출 정리
- [x] 테스트 3개 파일 업데이트 (45개 통과)
...
허용 헤더: Content-Type
자격 증명(credentials): false (API 키 세션 제거됨, D-014)
```
...
| 번역 | **Hunyuan-MT-7B** | DeepL, Google, Papago | WMT25 1위 (일한 30/31 언어쌍), 로컬 인퍼런스, 외부 API 키 불필요 |
...
> 최초 작성: 2026-03-26
> 현황 분석: 기존 comic-text-detector(YOLOv5s) + Real-ESRGAN(x4plus 실사 학습) 조합의 한계를 확인하고, 만화 특화 모델로의 교체 계획을 수립.

---

### 21.1 업스케일러 교체 — Real-ESRGAN anime_6B (Tier 1: 즉시 적용)

#### 배경
현재 `RealESRGAN_x4plus.pth`는 실사 사진 데이터로 학습되어 만화체 라인아트·텍스트에 부적합하다. 같은 Real-ESRGAN 패밀리의 `anime_6B` 변형은 애니메이션/만화 특화로 학습되었으며:

| 항목 | x4plus (현재) | x4plus_anime_6B (신규) |
|------|--------------|----------------------|
| 학습 데이터 | 실사 사진 위주 | 애니메이션/만화 특화 |
| RRDBNet 블록 수 | 23 blocks | **6 blocks** |
| 모델 크기 | 64 MB | **17 MB** |
| 텍스트 획 선명도 | 보통 | **우수** |
| 붓글씨(毛筆) 복원 | 흐릿해질 수 있음 | **라인 유지 우수** |
| 추론 속도 | 기준 | **약 2배 빠름** |

#### 적용 범위
- `preprocessor.py`: `_upsampler_x4` 를 anime_6B 로 교체 (또는 anime_6B를 기본으로 사용)
- `download.py`: anime_6B 가중치 다운로드 항목 추가
- 기존 x4plus도 유지 (설정으로 전환 가능)

#### 변경 파일
| 파일 | 변경 내용 |
|------|-----------|
| `server/download.py` | `MODELS` 리스트에 `RealESRGAN_x4plus_anime_6B.pth` 항목 추가 |
| `server/pipeline/preprocessor.py` | `RRDBNet(num_block=6)` + anime_6B 가중치 우선 로딩, 없으면 x4plus fallback |
| `server/config.py` | `upscaler_variant: str = "anime_6b"` 환경변수 추가 |
...
#### 완료 기준
- [x] anime_6B 가중치 다운로드 및 models/ 배치
- [x] `TestPreprocessor` 전체 통과 (3개 신규 테스트 포함)
- [x] 기존 x4plus fallback 동작 확인

---

### 21.2 말풍선 검출기 교체 — Magi v2 (Tier 2: 중기 계획)

#### 배경
Oxford VGG Lab이 2024년 발표한 manga 전용 통합 모델. 현재 `bubble_detector.py`의 YOLOv5s 기반 단순 bbox 검출을 대폭 확장한다.

#### 현재 vs Magi v2 기능 비교

| 기능 | comic-text-detector | Magi v2 |
|------|--------------------|----|
| 말풍선 bbox 검출 | ✅ | ✅ |
| 텍스트 방향(세로/가로) | ✅ | ✅ |
| 패널(컷) 감지 | ❌ | ✅ |
| 캐릭터 박스 검출 | ❌ | ✅ |
| 화자 연결(대사↔캐릭터) | ❌ | ✅ |
| 읽기 순서 | ❌ (별도 utils/reading_order.py) | ✅ 내장 |
| 챕터 전체 문맥 | ❌ | ✅ |
| HuggingFace 모델 | ❌ | ✅ (`ragavsachdeva/magiv2`) |

#### 영향 범위 (대규모 리팩터링)

```
교체 시 변경 필요 파일:
├── server/pipeline/bubble_detector.py  ← 신규 MagiBubbleDetector 클래스
├── server/pipeline/orchestrator.py    ← BubbleInfo 스키마 변경 대응
├── server/utils/reading_order.py       ← Magi 내장 읽기 순서로 대체 가능
├── server/schemas/models.py            ← 화자/패널 정보 스키마 추가
├── server/download.py                  ← HuggingFace 모델 자동 캐시
└── pyproject.toml                      ← transformers 의존성 추가
```

#### 신규 기능 (Magi v2 도입 시 활성화 가능)
- 화자 이름 자동 식별 → 번역 로그에 `speaker` 필드 추가
- 패널 단위 문맥 번역 (현재는 말풍선 단위)
- 챕터 전체 캐릭터 일관성 유지

#### 모델 크기 및 제약
| 항목 | 값 |
|------|-----|
| HuggingFace repo | `ragavsachdeva/magiv2` |
| 모델 크기 | ~1.2 GB (추정) |
| 추론 속도 | YOLOv5보다 느림 (Transformer 기반) |
| GPU VRAM 요구 | ~4 GB 이상 |
| 의존성 | `transformers>=4.40` |

#### 단계별 도입 전략 (점진적 교체)

```
Step 1. 병렬 실행 A/B 테스트
  - MagiBubbleDetector 클래스를 별도 파일로 작성
  - 설정 플래그 USE_MAGI_DETECTOR=true 로 전환 가능
  - 동일 이미지에 두 모델 결과 비교 로깅

Step 2. 읽기 순서 통합
  - Magi의 reading_order 출력을 reading_order.py 대체
  - 기존 utils/reading_order.py는 fallback으로 유지

Step 3. 화자 연결 활성화
  - BubbleInfo에 speaker_id, character_name 필드 추가
  - translation_log.json에 화자 정보 포함

Step 4. 완전 교체
  - A/B 테스트에서 Magi가 우세하면 comic-text-detector 제거
```

#### 완료 기준 (Tier 2)
- [x] `MagiDetector` 클래스 구현 (`server/pipeline/magi_detector.py`, BubbleInfo 호환)
- [x] bbox 형식 변환 `(x1,y1,x2,y2)` → `(x,y,w,h)` 구현
- [x] 읽기 순서 Magi 내장 순서 보존 + `sort_bubbles_rtl()` 바이패스
- [x] 별도 캐시 키 `"magi_detector"` 분리 (YOLOv5와 충돌 방지)
- [x] VRAM 4GB 미만 환경에서 YOLOv5 fallback 동작 확인
- [x] 6개 테스트 통과 (TestMagiDetector 5 + TestMagiCacheKeySeparation 1)
...
```
P0 (즉시) — anime_6B 적용
  이유: 코드 변경 최소 (모델 파일 1개 추가 + preprocessor.py 수정 ~20줄)
       붓글씨 희미함 문제(2026-03-25 확인) 직접 해결
       기존 테스트 전부 통과 예상

P1 (1주 이내) — Magi v2 A/B 테스트 준비
  이유: 대규모 리팩터링 전에 효과를 먼저 검증
       detection 품질 확인 후 전면 교체 결정

P2 (1개월 이내) — Magi v2 전면 교체
  이유: A/B 결과 기반, 화자 연결 기능 활성화
```
...
| 리스크 | 가능성 | 영향 | 대응 |
|--------|--------|------|------|
| anime_6B가 특정 만화 스타일에 오히려 부적합 | 낮음 | 중간 | x4plus/anime_6B 설정 전환 유지 |
| Magi v2 추론 속도가 실서비스에 너무 느림 | 중간 | 높음 | A/B 속도 벤치마크 필수, 배치 크기 조정 |
| Magi v2 VRAM 초과 (소형 GPU 환경) | 중간 | 높음 | VRAM 감지 후 YOLOv5 fallback |
| Magi v2 라이선스 제약 | 낮음 | 높음 | 상용 서비스 전 라이선스 재확인 필요 |
| transformers 의존성 버전 충돌 | 낮음 | 중간 | uv lock으로 pin 관리 |
